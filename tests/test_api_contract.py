...
def test_direct_hcc_recurrence_protein_endpoint_returns_pxd022881_evidence() -> None:
    response = client.get("/api/features/ALDH1A1/protein")
    assert response.status_code == 200
    payload = response.json()
    evidence = next(item for item in payload["protein_evidence"] if item["study_accession"] == "PXD022881")
    assert evidence["evidence_kind"] == "direct_transplant_protein_biomarker"
    assert evidence["primary_uniprot"] == "P00352"
    assert evidence["sample_scope"] == "hcc_tumor_explant_recurrence_discovery_subset"
    assert any(
        contrast["contrast_id"] == "subsequent_post_lt_hcc_recurrence_vs_no_post_lt_hcc_recurrence"
        for contrast in evidence["published_contrasts"]
    )
...
def test_direct_hcc_recurrence_proteomics_study_detail_is_available() -> None:
    response = client.get("/api/studies/PXD022881")
    assert response.status_code == 200
    payload = response.json()
    assert payload["processing_status"] == "protein_feature_direct_evidence_ready"
    assert payload["sample_summary"]["sample_count"] == 12
    assert payload["sample_summary"]["published_analysis_subset_sample_count"] == 11
    assert payload["sample_summary"]["by_clinical_state"]["subsequent_post_lt_hcc_recurrence"] == 7
    assert payload["sample_summary"]["by_clinical_state"]["no_post_lt_hcc_recurrence"] == 4
    assert payload["sample_summary"]["unlabeled_design_sample_count"] == 1


def test_direct_hcc_recurrence_proteomics_downloads_are_available() -> None:
    response = client.get("/api/studies/PXD022881/downloads")
    assert response.status_code == 200
    artifacts = {item["artifact"] for item in response.json()["downloads"]}
    assert "samples" in artifacts
    assert "sample_summary" in artifacts
...
def test_non_transcriptomic_layers_have_registered_external_sources() -> None:
    ...
    assert "PXD022881" in proteome["registered_accessions"]
    assert "PXD022881" in proteome["processed_accessions"]
...
def test_non_transcriptomic_layer_details_link_feature_artifacts() -> None:
    ...
    assert "data/processed/PXD022881/protein_features.json" in proteome["current_artifacts"]
...
def test_multiomics_source_registry_exposes_direct_hcc_recurrence_proteomics_layer() -> None:
    response = client.get("/api/multiomics-sources/PXD022881")
    assert response.status_code == 200
    payload = response.json()
    assert payload["source_type"] == "repository_accession"
    assert payload["local_status"] == "protein_feature_direct_evidence_ready"
    assert payload["sample_origins"] == ["recipient_liver_explant_tumor"]
    assert "subsequent_post_lt_hcc_recurrence" in payload["clinical_states"]
    assert "data/processed/PXD022881/protein_features.json" in payload["processed_artifacts"]
...
def test_post_lt_hcc_recurrence_use_case_links_pxd022881_proteomics() -> None:
    response = client.get("/api/use-cases/POST_LT_HCC_RECURRENCE")
    assert response.status_code == 200
    payload = response.json()
    assert payload["readiness"] == "direct_hcc_recurrence_proteomics_ready"
    assert "PXD022881" in payload["primary_datasets"]
    assert "ALDH1A1" in payload["markers"]
    assert any("11-sample published discovery subset" in line for line in payload["current_evidence"])
